...
El **panel Capas** contiene la pila de capas y los métodos abreviados para administrarlas. El **panel Capas** funciona en estrecha colaboración con el **panel Propiedades**: selecciona una capa del **panel Capas** para ver sus propiedades en el **panel Propiedades.**
...
1. La sección de herramientas contiene botones que puede utilizar para
   ...
   1. Quitar una capa
1. El **selector de modo de Fusión** te permite ajustar la forma en que se fusiona una capa con las capas inferiores. El **selector de modo de Fusión** solo está disponible cuando se selecciona una capa de material; los filtros no utilizan modos de fusión.
1. La **Pila de capas** contiene todas las capas que componen el recurso.
...
La pila de capas es la colección de materiales, filtros y otros recursos que componen su material actual. Al igual que en Photoshop y Substance 3D Painter, la pila de capas funciona desde la capa inferior primero hasta la capa superior al final. Esto significa que cada capa puede afectar a las capas que se encuentran por debajo de ella.

Hay algunas formas de administrar la pila de capas:

| Acciones | Cómo aplicar el |
| --- | --- |
| Añadir una capa | Arrastre un recurso del **panel Recursos** al área de visualización para agregarlo a la parte superior de la pila de capas. Arrastre un recurso del **panel Recursos** a la pila de capas para agregarlo a una ubicación específica de la pila de capas. Usa el botón **Agregar una capa** de la sección de herramientas para seleccionar un filtro de una lista. |
...
Una capa de material contiene información en varios canales y se puede fusionar con las capas inferiores. Las capas de material se muestran de forma ligeramente diferente según se encuentren en la parte inferior de la pila o no. Por ejemplo, en la imagen siguiente se muestra un material de roca arrastrado dos veces a la pila de capas; observe que la capa inferior no tiene ningún icono para controlar la fusión, mientras que la capa superior sí lo tiene.

![Capas de material en pila de capas, la capa superior tiene una opción de fusión.](../../assets/Material-Layer.png)
...
* Una capa de material que no se encuentre en la parte inferior de la pila se puede fusionar con las capas inferiores, por lo que puedes usar el **selector de modo de Fusión** para cambiar el modo de fusión. Además, aparece un **icono de mezcla** junto al **icono de capa**. Seleccione el **icono de fusión** para ajustar la configuración de fusión de la capa en función del modo de fusión seleccionado.
...
Puede arrastrar una imagen desde el explorador de archivos de su sistema a Sampler. Si ya hay capas en su pila de capas, esta se añadirá a la parte superior de la pila. Si no hay capas en la pila de capas, aparecerá un cuadro de diálogo en el que puede elegir cómo procesar la imagen:
