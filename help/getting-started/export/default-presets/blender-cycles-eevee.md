---
helpx_url: "https://helpx.adobe.com/substance-3d-sampler/getting-started/export/default-presets/blender-cycles-eevee.html"
breadcrumb-title: ''
description: Aprenda a exportar materiales desde Substance 3D Sampler utilizando el ajuste preestablecido Ciclos de fusión/Subida para la compatibilidad con Blender.
helpx_creative_field: ""
helpx_description: Sampler > Getting Started > Export > Default Presets > Blender CyclesEevee
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: Ciclos de licuadoraEeve
user-guide-description: ''
user-guide-title: ''
source-git-commit: 55277f7a92e97bf530dd2a2edf4e16c88bb57793
workflow-type: tm+mt
source-wordcount: '35'
ht-degree: 5%

---


# Ciclos de mezclador/Eeve

| Ajuste preestablecido | Compatibilidad | Descripción de salida de empaquetado |
| --- | --- | --- |
| Ciclos de mezclador/Eeve | <ul data-preserve-html="true"><li data-preserve-html="true">PBR Metálico/Rugosidad</li></ul> | **ColorBase ****Rugosidad****Metálico ****Desplazamiento****normal ****Emisivo****Alpha** |
